...
trait Sender[K, V] {
  ...
  def send(requestName: Expression[String], key: Option[Expression[K]], payload: Expression[V], headers: Option[Expression[util.List[Header]]]): RequestBuilder[K, V]
...
object Sender extends LowPriorSender {
  ...
  implicit def Avro4sSender[K, V](implicit
                                  ...
                                  headers: util.List[Header]): Sender[K, V] = new Sender[K, V] {
    ...
    override def send(requestName: Expression[String],
                      ...
                      headers: Option[Expression[util.List[Header]]]): RequestBuilder[K, V] =
